...
| Command | Description |
| --- | --- |
| `auth setpassword <UUID> <password> <confirmPassword>` | Set or reset the offline password for the specified UUID. |
| `auth mode set <username> <online\|offline>` | Set the login mode for the specified player, forcing premium or offline login for future joins. |
| `auth mode remove <username>` | Remove the specified player from the known player list, returning them to first-login state. |
